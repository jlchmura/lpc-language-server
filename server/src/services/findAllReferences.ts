...
export namespace Core {
    ...
    export function getReferencedSymbolsForNode(position: number, node: Node, program: Program, sourceFiles: readonly SourceFile[], cancellationToken: CancellationToken, options: Options = {}, sourceFilesSet: ReadonlySet<string> = new Set(sourceFiles.map(f => f.fileName))): readonly SymbolAndEntries[] | undefined {
        ...
        if (!symbol) {
            ...
            if (!options.implementations && isStringLiteral(node)) {
        ...
        const references = mapDefined<ModuleReference, Entry>(findModuleReferences(program, sourceFiles, symbol), reference => {
            if (reference.kind === "import") {
                ...
                if (isLiteralTypeNode(parent)) {
                    ...
                    //     return undefined;
                    // }
                    Debug.fail("TODO implement me - isLiteralTypeNode");
                }
                // import("foo") with no qualifier will reference the `export =` of the module, which may be referenced anyway.
                return nodeEntry(reference.literal);
            }
            else if (reference.kind === "implicit") {
                Debug.fail("TODO implement me - implicit");
    ...
    function getReferencedSymbolsSpecial(node: Node, sourceFiles: readonly SourceFile[], cancellationToken: CancellationToken): SymbolAndEntries[] | undefined {
        if (isTypeKeyword(node.kind)) {
            ...
            return getAllReferencesForKeyword(
                ...
                node.kind,
                cancellationToken,
                undefined//node.kind === SyntaxKind.ReadonlyKeyword ? isReadonlyTypeOperator : undefined,
            );
        }
    ...
    function getReferencedSymbolsForSymbol(originalSymbol: Symbol, node: Node | undefined, sourceFiles: readonly SourceFile[], sourceFilesSet: ReadonlySet<string>, checker: TypeChecker, cancellationToken: CancellationToken, options: Options): SymbolAndEntries[] {
        ...
        // }
        // else if (node && node.kind === SyntaxKind.DefaultKeyword && symbol.name === InternalSymbolName.Default && symbol.parent) {
    ...
    function shouldAddSingleReference(singleRef: Identifier | StringLiteral, state: State): boolean {
        ...
        // TODO
        return false;// !(isImportOrExportSpecifier(singleRef.parent) && singleRef.text === InternalSymbolName.Default);
    }
    ...
    function getSymbolScope(symbol: Symbol): Node | undefined {
        ...
        if (flags & (SymbolFlags.Property | SymbolFlags.Method)) {
            ...
            if (privateDeclaration) {
                return getAncestor(privateDeclaration, SyntaxKind.SourceFile);
            }
    ...
    export function eachSymbolReferenceInFile<T>(definition: Identifier, checker: TypeChecker, sourceFile: SourceFile, cb: (token: Identifier) => T, searchContainer: Node = sourceFile): T | undefined {
        ...
        for (const token of getPossibleSymbolReferenceNodes(sourceFile, symbol.name, searchContainer)) {
            if (!isIdentifier(token) || token === definition || token.text !== definition.text) continue;
    ...
    export function someSignatureUsage(
        ...
        for (const sourceFile of sourceFiles) {
            for (const name of getPossibleSymbolReferenceNodes(sourceFile, symbol.name)) {
                if (!isIdentifier(name) || name === signature.name || name.text !== signature.name.text) continue;
    ...
    function getReferencesAtLocation(sourceFile: SourceFile, position: number, search: Search, state: State, addReferencesHere: boolean): void {
        ...
        switch (state.specialSearchKind) {
            ...
            //     break;
            default:
                Debug.fail("special serach kind");//(state.specialSearchKind);
        }
    ...
    //         if (!(state.options.use === FindReferencesUse.Rename && (name.text === InternalSymbolName.Default))) {
    //             addRef();
    //         }
    //     }
    ...
    function getReferencesForSuperKeyword(superKeyword: Node): SymbolAndEntries[] | undefined {
        let searchSpaceNode: SuperContainer /*| ClassLikeDeclaration | InterfaceDeclaration*/ | TypeLiteralNode |  ObjectLiteralExpression | StructDeclaration | undefined = getSuperContainer(superKeyword, /*stopOnFunctions*/ false);
        if (!searchSpaceNode) {
            return undefined;
        }
    ...
    function isStringLiteralPropertyReference(node: StringLiteral, checker: TypeChecker) {
    ...
    function forEachRelatedSymbol<T>(
        ...
        if (!isForRenamePopulateSearchSymbolSet) {
            ...
            if (onlyIncludeBindingElementAtReferenceLocation) {
                bindingElementPropertySymbol = undefined;// isObjectBindingElementWithoutPropertyName(location.parent) ? getPropertySymbolFromBindingElement(checker, location.parent) : undefined;
            }
            else {
                bindingElementPropertySymbol = undefined;// getPropertySymbolOfObjectBindingPatternWithoutPropertyName(symbol, checker);
            }
        ...
        if (includeOriginalSymbolOfBindingElement) {
            const bindingElementPropertySymbol = undefined;// getPropertySymbolOfObjectBindingPatternWithoutPropertyName(symbol, checker);
            return bindingElementPropertySymbol && fromRoot(bindingElementPropertySymbol, EntryKind.SearchedPropertyFoundLocal);
        }
    ...
    function sharesDeclarationWithSearch(referenceSymbol: Symbol, search: Search): boolean {
        ...
        return search.allSearchSymbols.some(searchSymbol =>
            searchSymbol.declarations?.some(decl =>
                contains(refDeclarations, decl) || refDeclarations.some(refDecl => sameOriginDeclaration(refDecl, decl))));
    }

    /** The file a declaration textually comes from -- the origin header for an #include-inlined node. */
    function originFileName(node: Node): string {
        return isInExternalFileContext(node) ? getSourceFileOrIncludeOfNode(node).fileName : node.getSourceFile().fileName;
    }

    /**
     * True when two declaration nodes are the same source construct seen through different
     * parses. An `#include`d header is parsed twice -- once inlined into the includer (where
     * it merges with the implementation) and once as the header's own file -- producing two
     * distinct nodes for the same prototype. They share a kind, origin file, and header-
     * relative position, so match on that to unify them for find-references / rename.
     */
    function sameOriginDeclaration(a: Declaration, b: Declaration): boolean {
        return a !== b
            && a.kind === b.kind
            && a.pos === b.pos
            && a.end === b.end
            && originFileName(a) === originFileName(b);
    }
    ...
    function isImplementation(node: Node): boolean {
        return !!(node.flags & NodeFlags.Ambient) ? //? !(isInterfaceDeclaration(node) || isTypeAliasDeclaration(node)) :
            (isVariableLike(node) ? hasInitializer(node) :
                ...
                isClassLike(node)) : undefined;// || isModuleOrEnumDeclaration(node));
    }
