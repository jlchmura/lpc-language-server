...
export class ChangeTracker {
    ...
    private readonly classesWithNodesInsertedAtStart = new Map<number, { readonly node: ClassLikeDeclaration | ObjectLiteralExpression; readonly sourceFile: SourceFile; }>(); // Set<ClassDeclaration> implemented as Map<node id, ClassDeclaration>
    ...
    public static with(context: TextChangesContext, cb: (tracker: ChangeTracker) => void): FileTextChanges[] {
        ...
        cb(tracker);
        return tracker.getChanges(context);
    }
    ...
    public insertMemberAtStart(sourceFile: SourceFile, node: ClassLikeDeclaration | TypeLiteralNode, newElement: ClassElement | PropertySignature | MethodSignature): void {
    ...
    private insertNodeAtStartWorker(sourceFile: SourceFile, node: ClassLikeDeclaration  | ObjectLiteralExpression | TypeLiteralNode , newElement: ClassElement | ObjectLiteralElementLike | PropertySignature | MethodSignature ): void {
     ...
     */
    private guessIndentationFromExistingMembers(sourceFile: SourceFile, node: ClassLikeDeclaration  | ObjectLiteralExpression | TypeLiteralNode) {
    ...
    private computeIndentationForNewMember(sourceFile: SourceFile, node: ClassLikeDeclaration  | ObjectLiteralExpression | TypeLiteralNode) {
    ...
    private getInsertNodeAtStartInsertOptions(sourceFile: SourceFile, node: ClassLikeDeclaration  | ObjectLiteralExpression | TypeLiteralNode, indentation: number): InsertNodeOptions {
        // Rules:
    ...
    public getChanges(context: TextChangesContext, validate?: ValidateNonFormattedText): FileTextChanges[] {
        ...
        const driverType = host.getCompilationSettings()?.driverType;
        ...
        if (this.newFileChanges) {
            this.newFileChanges.forEach((insertions, fileName) => {
                changes.push(changesToText.newFileChanges(fileName, insertions, this.newLineCharacter, this.formatContext, fileHandler, driverType));
            });
        }
        return changes;
    }
...
function getClassOrObjectBraceEnds(cls: ClassLikeDeclaration  | ObjectLiteralExpression, sourceFile: SourceFile): [number | undefined, number | undefined] {
...
function getMembersOrProperties(node: ClassLikeDeclaration  | ObjectLiteralExpression | TypeLiteralNode ): NodeArray<Node> {
...
namespace changesToText {
    ...
    export function newFileChanges(fileName: string, insertions: readonly NewFileInsertion[], newLineCharacter: string, formatContext: formatting.FormatContext, fileHandler: LpcFileHandler, languageVariant: LanguageVariant): FileTextChanges {
        const text = newFileChangesWorker(getScriptKindFromFileName(fileName), languageVariant, fileHandler, insertions, newLineCharacter, formatContext);
        return { fileName, textChanges: [createTextChange(createTextSpan(0, 0), text)], isNewFile: true };
    }

    export function newFileChangesWorker(scriptKind: ScriptKind, languageVariant: LanguageVariant, fileHandler: LpcFileHandler, insertions: readonly NewFileInsertion[], newLineCharacter: string, formatContext: formatting.FormatContext): string {
        ...
        const sourceFile = createSourceFile("any file name", nonFormattedText, fileHandler, { languageVersion: ScriptTarget.LPC, jsDocParsingMode: JSDocParsingMode.ParseNone }, /*setParentNodes*/ true, scriptKind, languageVariant);
