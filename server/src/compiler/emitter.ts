...
export function createPrinter(printerOptions: PrinterOptions = {}, handlers: PrintHandlers = {}): Printer {
        ...
        if (hint === EmitHint.Unspecified) {
            switch (node.kind) {
                ...
                case SyntaxKind.PromiseType:
                    return emitPromiseType(node as PromiseTypeNode);
                ...
                case SyntaxKind.MappingTypeEntry:
                    return emitMappingTypeEntry(node as MappingTypeEntryNode);
    ...
    function emitPromiseType(node: PromiseTypeNode) {
        writeKeyword("promise");
        // Bare `promise` is `promise<mixed>`; print it the way it was written.
    ...
    function emitMappingType(node: MappingTypeNode) {
        ...
        node.entries?.forEach((entry, i) => {
            ...
            emitMappingTypeEntry(entry);
        });
        writeSpace();
        writePunctuation("])");
    }

    function emitMappingTypeEntry(node: MappingTypeEntryNode) {
        emit(node.keyType);
        node.elements?.forEach((value, i) => {
            writePunctuation(i === 0 ? ":" : ";");
            writeSpace();
            emit(value);
        });
    }
