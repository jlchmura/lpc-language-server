    ...
    while (true) {
        switch (parent.kind) {
            case SyntaxKind.BinaryExpression:
                ...
                // The key/value comma expression of `foreach (k, v in m)` assigns to both
                // operands, so keep walking up to reach the foreach statement itself.
                if (
                    binaryOperator === SyntaxKind.CommaToken && isForEachStatement(binaryExpression.parent) &&
                    binaryExpression.parent.initializer === binaryExpression
                ) {
                    node = parent;
                    break;
                }
...
export function isInExpressionContext(node: Node): boolean {
    ...
    switch (parent.kind) {
        ...
        case SyntaxKind.TemplateSpan:
            // The interpolated expression of a `${...}` span is in expression context.
            return (parent as TemplateSpan).expression === node;
        default:
...
export function getEffectiveTypeAnnotationNode(node: Node, originatingFile?: SourceFile): TypeNode | undefined {
    ...
    if (type && shouldJsDocTypeOverrideTypeNode(type)) {
        ...
        // If the declarator is an array (e.g. `object *mobs`) but the JSDoc @type only
