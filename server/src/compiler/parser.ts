...
export namespace LpcParser {
    ...
    function initState(
        ...
        macroEnv = createMacroEnvironment();
        macroEnv.define("__DIR__", createBuiltInMacro("__DIR__", `"${fileDir}"`));
        macroEnv.define("__FILE__", createBuiltInMacro("__FILE__", `"${fileName}"`));

        forEachEntry(_configDefines || emptyMap, (value, key) => {
            macroEnv.define(key, createBuiltInMacro(key, value));
        });
        ...
        switch (incomingToken) {
            default:
                ...
                if (allowMacroProcessing && (incomingToken == SyntaxKind.Identifier || (isNonReservedKeyword(incomingToken) && !inContext(NodeFlags.JSDoc)))) {
                    ...
                    } else if ((macro = macroEnv.lookup(tokenValue)) && macro.disabled !== true && macro.range) {
    ...
    function getPositionState(): PositionState {
        return {
            ...
            include: currentTopLevelIncludeDirective,
            // Snapshot only the macro fields that are read back (see
            // MacroPositionSnapshot). Cloning the whole Macro here was ~30% of parse
            // time on macro-heavy files, since this runs for every parse function.
            macro: currentMacro
                ? { name: currentMacro.name, pos: currentMacro.pos, end: currentMacro.end }
                : undefined,
    ...
    function parsingContextErrors(context: ParsingContext) {
        ...
        //         return parseErrorAtCurrentToken(Diagnostics.Enum_member_expected);
            ...
            case ParsingContext.MappingTypeValues:
    ...
    function isListElement(parsingContext: ParsingContext, inErrorRecovery: boolean): boolean {
        ...
        switch (parsingContext) {
            ...
            case ParsingContext.MappingTypeValues:
                // A value is a TYPE (TypeArguments' test) but the list ends at the next entry's
                // comma or the closing bracket (MappingEntryMembers' terminator). Neither
                // existing context pairs those, which is why this one exists.
                return token() === SyntaxKind.SemicolonToken || isStartOfType();
            // case ParsingContext.HeritageClauses:
    ...
    function parseUndefDirective(): UndefDirective {
        ...
        macroEnv.undef(identifier.text);
    ...
    function getRootMacro(macro: MacroPositionSnapshot): MacroPositionSnapshot {
        let rootMacro = macro;
    ...
    function parseBinaryExpressionRest(precedence: OperatorPrecedence, leftOperand: Expression, pos: PositionState): Expression {
        while (true) {
            ...
            reScanGreaterToken(); 
            const impliedStringConcat = canImplyStringConcat(leftOperand) && (token() == SyntaxKind.StringLiteral || token() === SyntaxKind.NoSubstitutionTemplateLiteral || token() === SyntaxKind.TemplateHead);
    ...
    function tryParseImpliedStringConcatExpression(expression: LeftHandSideExpression, pos: PositionState): LeftHandSideExpression {
        ...
        while (canImplyStringConcat(expression) && isStartOfImpliedStringConcat()) {
            const tokenNode = finishNode(factoryCreateToken(SyntaxKind.PlusToken), getPositionState());
            tempBinaryExpression = makeBinaryExpression(tempBinaryExpression ?? expression, tokenNode, parsePrimaryExpression(), pos);
        }
        if (tempBinaryExpression) {
    ...
    function canImplyStringConcat(expression: Node): boolean {
        return isStringLiteral(expression) || isBinaryExpression(expression) || isTemplateExpression(expression);
    }

    // FluffOS allows a template literal to sit adjacent to a string or another template
    // literal and implicitly concatenate (unlike JavaScript). These are the tokens that
    // begin such an adjacent operand.
    function isStartOfImpliedStringConcat(): boolean {
        return token() === SyntaxKind.StringLiteral
            || token() === SyntaxKind.StringizedIdentifier
    ...
    /**
     * FluffOS grammar (issue #1319):
     *      atomic_type: L_PROMISE | L_PROMISE '<' basic_type optional_star L_ORDER
     *
     * Bare `promise` is `promise<mixed>`. The payload is a basic type with optional `*`,
     * so `promise<int *>` is a promise of an array while `promise<int> *` is an array of
     * promises -- the `*` binds where it is written. A promise of a promise is not
     * spellable, matching the runtime's adoption of a promise resolved with a promise.
     *
     * The driver's closing `>` arrives as its shared comparison token; here the scanner
     * hands back GreaterThanToken directly. Nesting can't arise, so there is no `>>` to
     * split apart.
     */
    function parsePromiseTypeNode(): PromiseTypeNode {
        const pos = getPositionState();

        parseExpected(SyntaxKind.PromiseKeyword);

        let typeArgument: TypeNode | undefined;
        if (parseOptional(SyntaxKind.LessThanToken)) {
            typeArgument = parseType();
            parseExpected(SyntaxKind.GreaterThanToken);
        }

        return finishNode(factory.createPromiseTypeNode(typeArgument), pos);
    }
    ...
    function parseMappingTypeNode(): MappingTypeNode {
        ...
        // A comma separates ENTRIES, not value columns: `([ "a": int, "b": string ])` is two
        // keys. A value that may be one of several types is a union -- `([ string: int | float ])`
        // -- which is what the spec already uses, and mapping LITERALS separate their value
        // columns with `;`, so a comma meaning "column" here contradicted the literal syntax.
        const entries = parseDelimitedList(ParsingContext.TypeArguments, parseMappingTypeEntry);

        parseExpectedMatchingBracketTokens(SyntaxKind.OpenParenBracketToken, [SyntaxKind.CloseBracketToken, SyntaxKind.CloseParenToken], openBracketParsed, openBracketPosition, openBracketFilename);
        
        return finishNode(factory.createMappingTypeNode(entries), pos);
    }

    function parseMappingTypeEntry(): MappingTypeEntryNode {
        const pos = getPositionState();
        const keyType = parseType();
        let valueTypes: NodeArray<TypeNode> | undefined;

        if (parseOptional(SyntaxKind.ColonToken)) {
            // Several value types here are LDMud's multi-value mapping, whose columns the
            // literal syntax writes with `;`.
            valueTypes = parseTokenDelimitedList(ParsingContext.MappingTypeValues, parseType, SyntaxKind.SemicolonToken);
            if (valueTypes?.length === 0) {
                createMissingNode(SyntaxKind.MixedKeyword, false);                
            }
        }

        return finishNode(factory.createMappingTypeEntryNode(keyType, valueTypes ?? emptyArray), pos);
    }
    ...
    // Builds a StringLiteral node out of the current template literal token
    // (NoSubstitutionTemplateLiteral / TemplateHead / TemplateMiddle / TemplateTail)
    // and advances the scanner. The literal chunks of a template are represented as
    // plain StringLiteral nodes.
    function parseTemplateLiteralLikeString(): StringLiteral {
        const pos = getPositionState();
        const node = factoryCreateStringLiteral(internStringIfNeeded(scanner.getTokenValue()), /*isSingleQuote*/ undefined, scanner.hasExtendedUnicodeEscape());
        if (scanner.isUnterminated()) {
            node.isUnterminated = true;
        }
        nextToken();
    ...
    function parseTemplateExpression(isTaggedTemplate: boolean): TemplateExpression {
        const pos = getPositionState();
        const head = parseTemplateLiteralLikeString(); // consumes the TemplateHead token
        const templateSpans: TemplateSpan[] = [];
        let isTail = false;
        do {
            const span = parseTemplateSpan(isTaggedTemplate);
            templateSpans.push(span.node);
            isTail = span.isTail;
        } while (!isTail);
        return finishNode(factory.createTemplateExpression(head, templateSpans), pos);
    }

    function parseTemplateSpan(isTaggedTemplate: boolean): { node: TemplateSpan; isTail: boolean } {
        const pos = getPositionState();
        const expression = allowInAnd(parseExpression);
        let literal: StringLiteral;
        let isTail: boolean;
        if (token() === SyntaxKind.CloseBraceToken) {
            // Re-scan the '}' as the continuation of the template (TemplateMiddle or TemplateTail).
            currentToken = scanner.reScanTemplateToken(isTaggedTemplate);
            isTail = token() === SyntaxKind.TemplateTail;
            literal = parseTemplateLiteralLikeString();
        } else {
            parseExpected(SyntaxKind.CloseBraceToken);
            isTail = true;
            literal = createMissingNode<StringLiteral>(SyntaxKind.StringLiteral, /*reportAtCurrentPosition*/ false);
        }
        const node = finishNode(factory.createTemplateSpan(expression, literal), pos);
        return { node, isTail };
    }
    ...
    function parsePrimaryExpression(): PrimaryExpression {
        switch (token()) {
            case SyntaxKind.NoSubstitutionTemplateLiteral:
                return parseTemplateLiteralLikeString();
            case SyntaxKind.NumericLiteral:
            case SyntaxKind.IntLiteral:
            case SyntaxKind.CharLiteral:
            case SyntaxKind.FloatLiteral:
    ...
    const enum ParsingContext {
        ...
        MappingTypeValues,         // Value types of one mapping type entry, `;`-delimited
...
const forEachChildTable: ForEachChildTable = {
    ...
    [SyntaxKind.PromiseType]: function forEachChildInPromiseTypeNode<T>(node: PromiseTypeNode, cbNode: (node: Node) => T | undefined, _cbNodes?: (nodes: NodeArray<Node>) => T | undefined): T | undefined {
        return visitNode(cbNode, node.typeArgument);
    },
    ...
    [SyntaxKind.MappingType]: function forEachChildInMappingType<T>(node: MappingTypeNode, cbNode: (node: Node) => T | undefined, cbNodes?: (nodes: NodeArray<Node>) => T | undefined): T | undefined {
        return visitNodes(cbNode, cbNodes, node.entries);
    },
    [SyntaxKind.MappingTypeEntry]: function forEachChildInMappingTypeEntry<T>(node: MappingTypeEntryNode, cbNode: (node: Node) => T | undefined, cbNodes?: (nodes: NodeArray<Node>) => T | undefined): T | undefined {
