        ...
        function collectModuleReferences(file: SourceFile, node: Node, inAmbientModule: boolean): void {            
            ...
            //     }
                ...
                if ((isJSDocParameterTag(node) || isJSDocTypeTag(node) || isJSDocPropertyTag(node) || isJSDocReturnTag(node)) && node.typeExpression) { 
                    ...
                    if (jsDocType && isLiteralTypeNode(jsDocType)) {
                        if (isStringLiteral(jsDocType.literal)) {
                            imports = append(imports, jsDocType.literal);
                        } else if (isBinaryExpression(jsDocType.literal)) {
                            const parts = getStringLiteralsTextRecursively(jsDocType.literal);
                            if (parts.length) {                       
                                const lit = factory.createStringLiteral(parts.join(""));
