...
export function createProgram(rootNamesOrOptions: readonly string[] | CreateProgramOptions, _options?: CompilerOptions, _host?: CompilerHost, _oldProgram?: Program, _configFileParsingDiagnostics?: readonly Diagnostic[]): Program {
    ...
    function collectExternalModuleReferences(file: SourceFile): void {
        ...
        function getStringLiteralsFromTypeNodes(jsDocType: TypeNode): Array<StringLiteral|BinaryExpression> | undefined {
            ...
            } else if (isParenthesizedTypeNode(jsDocType) && jsDocType.type) {
                // `(STD_ITEM | STD_NPC)*`
                return getStringLiteralsFromTypeNodes(jsDocType.type);
            } else if (isMappingTypeNode(jsDocType) && jsDocType.entries?.length) {
                // A mapping's keys and values are types too: `([ STD_BODY: float ])` keys the
                // mapping by an object, and the path only resolves if its file enters the
                // dependency graph from here.
                return flatMap(jsDocType.entries, entry => [
                    ...(getStringLiteralsFromTypeNodes(entry.keyType) ?? []),
                    ...flatMap(entry.elements ?? [], (v: TypeNode) => getStringLiteralsFromTypeNodes(v) ?? []),
                ]) as Array<StringLiteral | BinaryExpression>;
            }
...
export function formatDiagnostic(diagnostic: Diagnostic, host: FormatDiagnosticsHost): string {
    const errorMessage = `${diagnosticSeverityName(diagnostic)} LPC${diagnostic.code}: ${flattenDiagnosticMessageText(diagnostic.messageText, host.getNewLine())}${host.getNewLine()}`;
...
export function formatDiagnosticsWithColorAndContext(diagnostics: readonly Diagnostic[], host: FormatDiagnosticsHost): string {
    ...
    for (const diagnostic of diagnostics) {
        ...
        output += formatColorAndReset(diagnosticSeverityName(diagnostic), getCategoryFormat(diagnostic.category));
