...
import { isBlock, isIdentifier, isParameter, isShorthandPropertyAssignment, isSpreadAssignment } from "./nodeTests";
import {
...
import { forEachReturnStatement, getEffectiveReturnTypeNode, getEffectiveTypeAnnotationNode, isDeclarationReadonly, isEntityNameExpression, isKeyword, isPrimitiveLiteralValue, isVarConstLike, nodeIsMissing } from "./utilities";
import { isConstTypeReference } from "./utilitiesPublic";

/** @internal */
export function createSyntacticTypeNodeBuilder(options: CompilerOptions, resolver: SyntacticTypeNodeBuilderResolver) {
    const strictNullChecks = true;//getStrictOptionValue(options, "strictNullChecks");
        ...
        switch (node.kind) {
            ...
            case SyntaxKind.PrefixUnaryExpression:
                ...
                if (isPrimitiveLiteralValue(unaryExpression)) {                    
                    if (unaryExpression.operand.kind === SyntaxKind.IntLiteral || unaryExpression.operand.kind === SyntaxKind.FloatLiteral) {
    ...
    function typeFromSingleReturnExpression(declaration: FunctionLikeDeclaration | undefined, context: SyntacticTypeNodeBuilderContext): boolean | undefined {
        ...
        if (declaration && !nodeIsMissing(declaration.body)) {
            ...
            if (body && isBlock(body)) {
                ...
                candidateExpr = body as Expression; // TODO: that "as" shouldn't be necessary
            }
        }
        if (candidateExpr) {
