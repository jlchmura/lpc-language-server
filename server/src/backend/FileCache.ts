/**
 * Copied from https://github.com/microsoft/TypeScript/blob/ddf43cd0e0091057254b1fcfcaf880140dfb7ea1/src/server/scriptVersionCache.ts
...
class EditWalker implements LineIndexWalker {
        ...
        if (lines.length > 0) {
            ...
            if (lines.length > 1) {
                ...
                while (pathIndex >= 0) {
                    ...
                    insertedNodes = insertionNode.insertAt(
                        startNode,
                        insertedNodes
                    );
                    pathIndex--;
    ...
    leaf(relativeStart: number, relativeLength: number, ll: LineLeaf) {
        if (this.state === CharRangeSection.Start) {
            ...
            this.initialText = ll.text.substring(0, relativeStart);
            this.trailingText = ll.text.substring(
                relativeStart + relativeLength
            );
        } else {
...
export class ScriptVersionCache {
    ...
    lineToTextSpan(line: number): TextSpan {
        ...
        const len =
            lineText !== undefined
                ? lineText.length
                : index.absolutePositionOfStartOfLine(line + 2) -
                  absolutePosition;
...
class LineIndexSnapshot implements IScriptSnapshot {
    constructor(
        ...
        readonly changesSincePreviousVersion: readonly TextChange[] = []
    ) {}
...
export class LineIndex {
    ...
    positionToLineOffset(position: number): protocol.Location {
        const { oneBasedLine, zeroBasedColumn } =
            this.root.charOffsetToLineInfo(1, position);
    ...
    lineNumberToInfo(oneBasedLine: number): AbsolutePositionAndLineText {
        ...
        if (oneBasedLine <= lineCount) {
            const { position, leaf } = this.root.lineNumberToInfo(
                oneBasedLine,
                0
            );
    ...
    private static buildTreeFromBottom(nodes: LineCollection[]): LineNode {
        ...
        for (let i = 0; i < interiorNodes.length; i++) {
            const end = Math.min(
                nodeIndex + lineCollectionCapacity,
                nodes.length
            );
...
export class LineNode implements LineCollection {
    ...
    insertAt(child: LineCollection, nodes: LineCollection[]) {
        ...
        if (
            ...
            childIndex++;
            while (
                childIndex < lineCollectionCapacity &&
                nodeIndex < nodeCount
            ) {
