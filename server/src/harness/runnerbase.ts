...
export abstract class RunnerBase {
    ...
    public enumerateFiles(folder: string, regex?: RegExp, options?: { recursive: boolean; }): string[] {
        return lpc.map(IO.listFiles(userSpecifiedRoot + folder, regex, { recursive: (options ? options.recursive : false) }), ts.normalizeSlashes);
    }
    ...
    static removeFullPaths(path: string) {
        ...
        let fixedPath = /^(\w:|\/)/.test(path) ? lpc.getBaseFileName(path) : path;
