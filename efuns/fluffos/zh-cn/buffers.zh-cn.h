...
/**
 * to_buffer() - 将一个字符串或整数数组转换为缓冲区
 *
 * 返回一个包含 <x> 的字节的缓冲区：
 *
 * - 字符串贡献其原始 UTF-8 字节（一个多字节字符会变成多个字节）。
 * - 数组必须只包含 0..255 范围内的整数；每个元素变成一个字节。任何其他项都会引发错误。
 * - 缓冲区将原样返回。
 *
 * 该 efun 也是编译器使用的隐式提升，因此在赋值上下文中，凡是需要缓冲区的地方，
 * 都可以直接使用字符串或整数数组：
 *
 * ```c
 * buffer b;
 *
 * b = to_buffer("é");        // ({ 0xC3, 0xA9 }) -- 2 个 UTF-8 字节
 * b = to_buffer(({ 7, 255 }));
 * b = to_buffer(({ 300 }));  // 错误：项超出 0..255 范围
 * ```
 *
 */
