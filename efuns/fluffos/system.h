 ...
 *
 * Returns an array of filenames of every file the object's program actually
 * `#include`d when it was compiled -- nested includes included, first-seen
 * order, duplicates dropped. The object's own source file is omitted. Each
 * filename carries a leading slash.
 *
 * The configured global include file is listed when the compiler opened it.
 * An `#include` inside a false `#if` branch is NOT listed: that file was
 * never opened.
 *
 ...
 *
 * @see inherit_list, deep_inherit_list, recompile_object
 */
string *include_list( void|object obj );
